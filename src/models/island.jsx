...
import { useModelContext } from '../components/mdlcontext';
...
export function Model({ moving, setMoving, aircraftRef,skyRef, ...props }) {
  ...
  const handlePointerMove = (e) => {
    ...
    if (isRotating && moving) {
      ...
      group.current.rotation.y += deltax * 0.01 * Math.PI;
      group.current.rotation.x += deltay * 0.01 * Math.PI;
  ...
  }, [gl, handlePointerDown, handlePointerMove, handlePointerUp, handleKeyDown, handleKeyUp, isRotating, currentIndex]);

  useEffect(() => {
    // Function to update the rotation based on the current index
    const updateRotation = () => {
      if (coordinates[currentIndex] && group.current) {
        const { x: targetX, y: targetY } = coordinates[currentIndex];

        // Animate the island rotation
        gsap.to(group.current.rotation, {
          ...
          onUpdate: () => {
            ...
            // Update the sky orientation during the animation
            if (skyRef.current) {
  ...
  const normalizeAngle = (angle) => {
    return angle >= 0 ? angle % (2 * Math.PI) : (2 * Math.PI + angle % (2 * Math.PI)) % (2 * Math.PI);
  };
  ...
  useFrame(() => {
    if (!isRotating) {
      ...
      setRotationX(group.current.rotation.x);
    }
  }, [isRotating]);

  const checkCurrentStage = (rotationx, rotationy) => {
    const normalizedRotationX = normalizeAngle(rotationx);
    const normalizedRotationY = normalizeAngle(rotationy);

    const area = areasOfInterest.find(area =>
      normalizedRotationY >= area.rotationYRange[0] && normalizedRotationY <= area.rotationYRange[1] &&
      normalizedRotationX >= area.rotationXRange[0] && normalizedRotationX <= area.rotationXRange[1]
    );
